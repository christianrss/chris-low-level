//! PEDAGOGY-SOLUTION: CLVM-RS-FNV-01
//! PEDAGOGY-SOLUTION: CLVM-RS-HEADER-01
//! PEDAGOGY-SOLUTION: CLVM-RS-WALK-01
...
fn validate(data: &[u8]) -> Result<(), String> {
    ...
    if data[5] != 0 {
        return Err("unsupported flags".into());
    }
    ...
    if size != 0 && entry >= size {
        return Err("entry outside code".into());
    }
    let code = &data[16..];
    if fnv1a32(code) != expected {
...
mod tests {
    ...
    fn rejects_bad_magic() {
        ...
        buf[4] = 1;
        assert!(validate(&buf).unwrap_err().contains("magic"));
    }
    ...
    fn rejects_truncated_push() {
        ...
        assert!(validate(&buf).unwrap_err().contains("truncated"));
    }
