...
int main() {
    ...
    for (int frame = 0; frame < frames; ++frame) {
        const std::vector<Layer> layers = {{&window_a, 20 + frame, 30}, {&window_b, 150, 80 + frame / 2}};
        const Surface output = Compositor::compose(width, height, Pixel{20, 20, 20, 255}, layers);
        const Pixel sample = output.pixel(frame % width, frame % height);
        checksum += sample.r + sample.g + sample.b;
    }
    const auto end = std::chrono::steady_clock::now();
    const double seconds = std::chrono::duration<double>(end - start).count();
    std::cout << "frames=" << frames << " seconds=" << seconds << " fps=" << frames / seconds << " checksum=" << checksum << "\n";
}
