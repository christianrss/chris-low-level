# Teoria passo a passo — Triage de magic bytes

Este laboratório é em **Python**.

## 1. O que estamos construindo

Detecta ELF/PE/WASM pelos magics e valida tamanho mínimo — porta de entrada de malware analysis.
...
- Ordem de match errada.
- PE sem MZ.
- min_size inventado.

## 8. Lab versus produção

file(1), die, Detect It Easy.

## Modelo mental

prefixos → enum de formato.

## Por quê min_size?

Magic sozinho não basta se buffer truncado.

## Por quê PE=2?

Só `MZ` já identifica o stub mínimo do lab.

## Invariante

detect_magic None/unknown se não casar.

## Ligação

Dia 09 yara_match_scan; Dia 08 pe_export.

## Segurança

Não executar o binário — só classificar.
...
### Por quê medir?
O `BENCHMARK_GUIADO.md` pede a métrica `1e5 detect_magic` — mesmo que o ambiente
...
Neste módulo `redteam/capstone_triage`, o fluxo de dados não é abstrato: cada função do starter
