...
Implemente `IsPeFile` em `starter/PeExportSpan.cs`.
Critério de aceite: o `PEDAGOGY-TEST` ligado a `DN-PE-EXP-01` PASS; valores iguais ao paper-trace.

## Difícil — `DN-PE-EXP-02` + `DN-PE-EXP-03`

Complete `TryGetPeOffset` e `TryReadExportRva` mantendo invariantes dos passos anteriores.
