# Testes guiados - CPU: fetch/decode/execute
...
A solucao limpa e seus testes estao em `solutions/` e em `projects/chris-cpu/`. Leia os testes somente depois de tentar escrever sua propria versao.
...
- `CPU-STEP-01` — coberto pela sequência de testes/validação descrita neste arquivo; a solução correspondente também é verificada pelo `pedagogy_check`.

Arquivos de teste automatizado presentes no starter:
- `starter/tests/test_cpu.cpp`
