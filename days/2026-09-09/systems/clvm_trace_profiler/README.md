# Profiler de opcodes CLVM (C)

**Dia:** 2026-09-09 · **Trilha:** `systems` · **Módulo:** `clvm_trace_profiler`  
**Linguagem:** C  
**TODOs:** `CLVM-TRACE-01`, `CLVM-TRACE-02`, `CLVM-TRACE-03`

## Pré-requisitos

- Paper-trace da TEORIA (seção 4) feito no caderno
- Checkpoint correspondente em `ATIVIDADES.md` do dia

## Fluxo

1. `TEORIA_PASSO_A_PASSO.md`
2. `EXERCICIOS.md`
3. Implemente `starter/`
4. `TESTES_GUIADOS.md` / teste local
5. `RESOLUCAO_GUIADA_PASSO_A_PASSO.md` só se travar
6. `BENCHMARK_GUIADO.md`

## Ideia em uma frase

O disassembler do Dia 08 lista instruções. Este lab conta qual opcode aparece mais no bytecode — o primeiro passo de um profiler amostral.
