# Teoria passo a passo — Duplex stream pipeline

Este laboratório é em **JavaScript**.

## 1. O que estamos construindo

Capstone Node: pipe transforma chunks (`HELLO`), métricas de chunks, flush rejeita estado inválido.
...
1. transform/duplex conforme TODOs.
2. conte chunks.
...
Node stream.Duplex / pipeline().

## Modelo mental

chunk in → transform → chunk out + metrics.

## Por quê HELLO?

Fixture estável do teste.

## Por quê chunks==1?

Um write/push no Caso.

## Invariante

metrics reflete writes observados.

## Ligação

Dia 08 duplex_event_pipe; Dia 09 async_hooks.

## Flush

Recusar se contrato do lab não satisfeito.
...
### Por quê medir?
O `BENCHMARK_GUIADO.md` pede a métrica `1e4 chunks` — mesmo que o ambiente
...
Neste módulo `nodejs/capstone_stream_pipeline`, o fluxo de dados não é abstrato: cada função do starter
