# Teoria passo a passo — Verify + replay log FSM (Python)
...
- Não appendar dict estruturado.
- Ignorar payload.ok.
- Hash com len ≠ 16.

## 8. Lab versus produção

LangSmith/agent traces; aqui FSM mínima determinística.

## Modelo mental

log append-only + redução para estado.

## Por quê repr no hash?

Estável o bastante para o lab; produção usaria canonical JSON.

## Por quê REVISE?

Verify falhou — o loop deve corrigir, não mentir DONE.

## Invariante

append não muta entradas antigas.

## Ligação

Dia 10 capstone_agent_loop fecha o ciclo completo.

## Teste

start+verify ok → DONE.
...
### Por quê medir?
O `BENCHMARK_GUIADO.md` pede a métrica `1e5 append+hash` — mesmo que o ambiente
...
Neste módulo `agent/verify_replay_log`, o fluxo de dados não é abstrato: cada função do starter
