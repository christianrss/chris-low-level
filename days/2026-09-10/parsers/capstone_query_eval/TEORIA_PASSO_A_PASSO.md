# Teoria passo a passo — Query Pratt/bool simples

Este laboratório é em **Python**.

## 1. O que estamos construindo

Lex `a:1 AND b:2` e avalia booleanos — capstone do arco de parsers.
...
Elastic/Lucene query string; aqui subset.

## Modelo mental

tokens → árvore/bool → bool.

## Por quê AND True no fixture?

Termos a:1 e b:2 são verdadeiros no ambiente do teste.
...
lex ∘ eval estável para as strings do teste.

## Ligação

Dia 07 pratt_query_lang; Dia 09 logfmt.

## Paper

Liste os 3 tokens antes de codar.
...
### Por quê medir?
O `BENCHMARK_GUIADO.md` pede a métrica `1e5 eval_query` — mesmo que o ambiente
...
Neste módulo `parsers/capstone_query_eval`, o fluxo de dados não é abstrato: cada função do starter
