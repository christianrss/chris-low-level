# Resolução guiada — Detect de formato por assinatura

## Mapa exato starter → resolução

| TODO ID | Arquivo | Função / âncora |
|---------|---------|-----------------|
| `CAP-TOOL-DET-01` | `starter/capstone_format_detect.py` | `match_signature` |
| `CAP-TOOL-DET-02` | `starter/capstone_format_detect.py` | `confidence` |
| `CAP-TOOL-DET-03` | `starter/capstone_format_detect.py` | `detect_format` |

> Raiz: `days/2026-09-10/tooling/capstone_format_detect/starter/`. Tente sozinho antes de usar esta página.

## Baseline

```powershell
cd days/2026-09-10/tooling/capstone_format_detect/starter
python -m pytest -q 2>$null; if (-not $?) { python test_capstone_format_detect.py 2>$null; if (-not $?) { Get-ChildItem test*.py | ForEach-Object { python $_.FullName } } }
```

**Esperado antes dos TODOs:** FAIL (NotImplemented, stub, assert).


## CAP-TOOL-DET-01

### Onde colocar (CAP-TOOL-DET-01)

| Campo | Valor |
|-------|-------|
| Arquivo | `starter/capstone_format_detect.py` |
| Função / âncora | `match_signature` — comentário `TODO [CAP-TOOL-DET-01]` |
| Substituir | o **corpo** sob o TODO (mantenha a assinatura) |
| Não mexer | outros TODOs neste passo; arquivos de teste |

### O problema

Sem `CAP-TOOL-DET-01`, o Caso correspondente falha: PNG match.

### Algoritmo / trace

Use o trace da TEORIA (seção 4) e o trecho de `CAP-TOOL-DET-01` abaixo. Confira o número
no papel antes de colar.

### Escreva o código

```python
def match_signature(data: bytes) -> str | None:
    # PEDAGOGY-SOLUTION: CAP-TOOL-DET-01
    for sig, name in SIGNATURES:
        if data.startswith(sig):
            return name
    return None
```

### Por que funciona?

Por quê este corpo satisfaz `CAP-TOOL-DET-01`: ele implementa exatamente o contrato do
teste (PNG match.), sem atalho que ignore o wire da TEORIA.

### Verifique

Rode o teste do starter focando `CAP-TOOL-DET-01`. Esperado: este caso PASS; os TODOs
seguintes ainda podem FAIL.

## CAP-TOOL-DET-02

### Onde colocar (CAP-TOOL-DET-02)

| Campo | Valor |
|-------|-------|
| Arquivo | `starter/capstone_format_detect.py` |
| Função / âncora | `confidence` — comentário `TODO [CAP-TOOL-DET-02]` |
| Substituir | o **corpo** sob o TODO (mantenha a assinatura) |
| Não mexer | outros TODOs neste passo; arquivos de teste |

### O problema

Sem `CAP-TOOL-DET-02`, o Caso correspondente falha: format PNG.

### Algoritmo / trace

Use o trace da TEORIA (seção 4) e o trecho de `CAP-TOOL-DET-02` abaixo. Confira o número
no papel antes de colar.

### Escreva o código

```python
def confidence(data: bytes, fmt: str) -> float:
    # PEDAGOGY-SOLUTION: CAP-TOOL-DET-02
    for sig, name in SIGNATURES:
        if name == fmt:
            return 1.0 if data.startswith(sig) else 0.0
    return 0.0
```

### Por que funciona?

Por quê este corpo satisfaz `CAP-TOOL-DET-02`: ele implementa exatamente o contrato do
teste (format PNG.), sem atalho que ignore o wire da TEORIA.

### Verifique

Rode o teste do starter focando `CAP-TOOL-DET-02`. Esperado: este caso PASS; os TODOs
seguintes ainda podem FAIL.

## CAP-TOOL-DET-03

### Onde colocar (CAP-TOOL-DET-03)

| Campo | Valor |
|-------|-------|
| Arquivo | `starter/capstone_format_detect.py` |
| Função / âncora | `detect_format` — comentário `TODO [CAP-TOOL-DET-03]` |
| Substituir | o **corpo** sob o TODO (mantenha a assinatura) |
| Não mexer | outros TODOs neste passo; arquivos de teste |

### O problema

Sem `CAP-TOOL-DET-03`, o Caso correspondente falha: confidence 1.0.

### Algoritmo / trace

Use o trace da TEORIA (seção 4) e o trecho de `CAP-TOOL-DET-03` abaixo. Confira o número
no papel antes de colar.

### Escreva o código

```python
def detect_format(data: bytes) -> dict:
    # PEDAGOGY-SOLUTION: CAP-TOOL-DET-03
    fmt = match_signature(data)
    if fmt is None:
        return {"format": None, "confidence": 0.0}
    return {"format": fmt, "confidence": confidence(data, fmt)}
```

### Por que funciona?

Por quê este corpo satisfaz `CAP-TOOL-DET-03`: ele implementa exatamente o contrato do
teste (confidence 1.0.), sem atalho que ignore o wire da TEORIA.

### Verifique

Rode o teste do starter focando `CAP-TOOL-DET-03`. Esperado: este caso PASS; os TODOs
seguintes ainda podem FAIL.


## Debug

| Sintoma | Causa provável | Correção |
|---------|----------------|----------|
| NotImplemented / stub | corpo não substituído | cole o bloco do TODO |
| número/string diferente | trace errado no papel | refaça a seção 4 da TEORIA |
| caso seguinte quebra | mudou assinatura ou estado global | restaure o que “Não mexer” pede |
| listener/null (.NET) | sem ActivityListener | veja TESTES_GUIADOS |

## Checkpoint intermediário de integração

Depois do primeiro TODO que compila:

1. Rode o teste do módulo a partir de `starter/` (ou o comando do Baseline).
2. Confirme que o Caso 1 ainda falha **só** nos TODOs restantes (não por link quebrado).
3. Anote a mensagem de assert: ela aponta o próximo ID.

## Passo a passo de edição (operacional)

Para cada TODO restante, repita:

1. Abra o arquivo da tabela **Onde colocar**.
2. Localize o comentário `TODO [ID]` — não busque pelo nome do módulo na pasta pai.
3. Substitua **apenas** o corpo indicado; preserve assinatura e includes.
4. Compile; se o erro for de tipo/assinatura, você editou demais.
5. Só então avance ao próximo ID.

## Tabela de regressão rápida

| Depois de | Deve passar | Ainda pode falhar |
|-----------|-------------|-------------------|
| 1º TODO | asserts só desse ID | IDs seguintes |
| 2º TODO | IDs 1–2 | IDs seguintes |
| último TODO | suite inteira | — |

## Armadilhas específicas deste starter

- Mudar o teste para “passar” invalida o lab.
- Criar um segundo `.c`/`.py` com o mesmo símbolo gera link duplicado ou import errado.
- Reset ausente entre casos deixa estado (anel, FSM, arena) contaminado.

## Relatório — campos extras

Além do template padrão, anote:

- Tempo até o primeiro Caso 1 verde:
- Quantas vezes o endianness/size foi a causa:
- Um invariante que você quase violou:

## Relatório de resolução

- TODOs concluídos:
- Comando de teste:
- Saída observada:
- Invariantes checadas:
- Edge cases:
- Benchmark (`1e5 match_signature`):
