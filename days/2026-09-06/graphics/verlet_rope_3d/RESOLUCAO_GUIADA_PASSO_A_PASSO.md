...
| `GFX-PORTAL-SW-07` | `starter/software_win32/main.cpp` | wireframe CPU DIB |
...
> Trabalhe em `days/2026-09-06/graphics/verlet_rope_3d/starter/`. `solutions/` é gabarito — só depois da tentativa.
...
```powershell
cd E:\Aulas\low-level-unified-portfolio\days\2026-09-06\graphics\verlet_rope_3d\starter
...
| | |
|--|--|
| **Arquivo** | `starter/src/math.cpp` |
| **Função / âncora** | ``mat4_inverse_rigid`, `mat4_transform_point`, `mat4_transform_direction` — `TODO [GFX-PORTAL-01]`` — comentário `TODO [GFX-PORTAL-01]` |
| **Substituir** | o stub / corpo / case marcado por `TODO [GFX-PORTAL-01]` |
| **Não mexer** | demais arquivos do starter até este ID passar nos testes |
...
Rode os testes do módulo com `TODO [GFX-PORTAL-01]` implementado.

**Esperado:** caso(s) em TESTES_GUIADOS que cobrem `GFX-PORTAL-01` passam.
...
| | |
|--|--|
| **Arquivo** | `starter/src/portal.cpp` |
| **Função / âncora** | ``make_frame_matrix`, `make_portal_pair` — `TODO [GFX-PORTAL-02]`` — comentário `TODO [GFX-PORTAL-02]` |
| **Substituir** | o stub / corpo / case marcado por `TODO [GFX-PORTAL-02]` |
| **Não mexer** | demais arquivos do starter até este ID passar nos testes |
...
Rode os testes do módulo com `TODO [GFX-PORTAL-02]` implementado.

**Esperado:** caso(s) em TESTES_GUIADOS que cobrem `GFX-PORTAL-02` passam.
...
|--|--|
| **Arquivo** | `starter/src/portal.cpp` |
| **Função / âncora** | ``portal_transport_position`, `portal_transport_velocity` — `TODO [GFX-PORTAL-03]`` — comentário `TODO [GFX-PORTAL-03]` |
| **Substituir** | o stub / corpo / case marcado por `TODO [GFX-PORTAL-03]` |
| **Não mexer** | demais arquivos do starter até este ID passar nos testes |
...
Rode os testes do módulo com `TODO [GFX-PORTAL-03]` implementado.

**Esperado:** caso(s) em TESTES_GUIADOS que cobrem `GFX-PORTAL-03` passam.
...
| | |
|--|--|
| **Arquivo** | `starter/src/verlet.cpp` |
| **Função / âncora** | ``verlet_step` — `TODO [GFX-PORTAL-04]`` — comentário `TODO [GFX-PORTAL-04]` |
| **Substituir** | o stub / corpo / case marcado por `TODO [GFX-PORTAL-04]` |
| **Não mexer** | demais arquivos do starter até este ID passar nos testes |
...
Rode os testes do módulo com `TODO [GFX-PORTAL-04]` implementado.

**Esperado:** caso(s) em TESTES_GUIADOS que cobrem `GFX-PORTAL-04` passam.

### Onde colocar

| | |
|--|--|
| **Arquivo** | `starter/src/sphere_portal.cpp` |
| **Função / âncora** | ``sphere_crosses_portal_plane`, `try_sphere_teleport` — `TODO [GFX-PORTAL-05]`` — comentário `TODO [GFX-PORTAL-05]` |
| **Substituir** | o stub / corpo / case marcado por `TODO [GFX-PORTAL-05]` |
| **Não mexer** | demais arquivos do starter até este ID passar nos testes |
...
| | |
|--|--|
| **Arquivo** | `starter/src/render_gl.cpp` |
| **Função / âncora** | `comentário `TODO [GFX-PORTAL-06]` em `render_gl.cpp`` — comentário `TODO [GFX-PORTAL-06]` |
| **Substituir** | o stub / corpo / case marcado por `TODO [GFX-PORTAL-06]` |
| **Não mexer** | demais arquivos do starter até este ID passar nos testes |
...
```powershell
cd E:\Aulas\low-level-unified-portfolio\days\2026-09-06\graphics\verlet_rope_3d\solutions
cmake -S . -B build -A x64
cmake --build build --config Release
ctest --test-dir build -C Release --output-on-failure
```
...
## GFX-PORTAL-SW-07 — wireframe CPU
...
| Função | `wWinMain` + raster wireframe |
| Substituir | `TODO [GFX-PORTAL-SW-07]` |

### 1. O problema (SW-07)

Reproduzir sala, corda e esfera em wireframe CPU antes do stencil GL — paridade visual com `portal_demo`.

### Escreva o código (SW-07)

```cpp
// PEDAGOGY-SOLUTION: GFX-PORTAL-SW-07
// DIB framebuffer + draw_line para arestas da sala e segmentos Verlet
```

### Por que funciona (SW-07)

Mesmo `portal_core` avança física; só a apresentação muda — padrão dual-backend do Dia 01.

### Verifique (SW-07)

VISUAL-01: `portal_sw` e `portal_demo` mostram esfera na mesma posição após teleporte.
