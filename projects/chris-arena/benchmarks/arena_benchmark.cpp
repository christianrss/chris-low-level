...
int main() {
    ...
    const auto arena_ns = std::chrono::duration_cast<std::chrono::nanoseconds>(arena_end - arena_start).count();
    const auto heap_ns = std::chrono::duration_cast<std::chrono::nanoseconds>(heap_end - heap_start).count();
