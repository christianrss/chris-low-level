...
static std::uint32_t read_u32(const std::vector<std::uint8_t>& bytes, std::size_t offset) {
    ...
    for (int shift = 0; shift < 32; shift += 8) value |= static_cast<std::uint32_t>(bytes[offset++]) << shift;
    return value;
}
...
std::vector<std::uint8_t> encode_debug_packet(const DebugPacket& packet) {
    if (packet.payload.size() > 1024 * 1024) throw std::invalid_argument("debug packet payload too large");
...
DebugPacket decode_debug_packet(const std::vector<std::uint8_t>& bytes) {
    ...
    if (bytes.size() != kHeaderSize + payload_size) throw std::runtime_error("debug packet length mismatch");
    ...
    if (actual_hash != expected_hash) throw std::runtime_error("debug packet checksum mismatch");
