# Teoria passo a passo — ActivitySource e spans (.NET)
...
- Esquecer o listener: StartActivity pode devolver null.
- Tag com nome errado.
- Export sem pipe.

## 8. Lab versus produção

Em produção exporta OTLP/Jaeger; aqui o export é string para assert estável.

## Modelo mental

Source → Activity (span) → tags → export.

## Por quê precisa de ActivityListener?

Sem sampling, CreateActivity/StartActivity retorna null.

## Por quê tag module fixa?

O assert compara o literal do lab, não um valor dinâmico.

## Invariante

Export usa OperationName e a tag module, nessa ordem.

## Ligação

Mesmo conceito de span que Node async_hooks rastreia por fase.

## dotnet test

Projeto de testes referenciando Chris.ActivityLab.
...
### Por quê medir?
O `BENCHMARK_GUIADO.md` pede a métrica `1e5 StartWorkSpan+Export` — mesmo que o ambiente
...
Neste módulo `dotnet/activity_source_span`, o fluxo de dados não é abstrato: cada função do starter
